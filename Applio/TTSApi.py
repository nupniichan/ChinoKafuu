...
async def text_to_speech(request: TTSRequest):
    ...
    if not os.path.exists(pth_path) or not os.path.exists(index_path):
        raise HTTPException(status_code=404, detail="Model chưa tồn tại.")
    ...
    safe_export_format = request.export_format if request.export_format in ["wav", "mp3", "ogg"] else "wav"
    output_rvc_path = safe_path_join(
        guild_output_dir,
        f"result_{timestamp}.{safe_export_format}",
    )

    os.makedirs("tts_temp", exist_ok=True)
    os.makedirs("tts_output", exist_ok=True)

    if torch.cuda.is_available() and request.gpu >= 0:
        torch.cuda.set_device(request.gpu)
        config = Config()
        config.device = f"cuda:{request.gpu}"
        if config.device.startswith("cuda"):
            config.set_cuda_config()
