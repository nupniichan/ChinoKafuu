    ...
    public class OsuSlashCommand : ApplicationCommandModule
    {
            ...
            try
            {
                ...
                if (users != null && users.Length > 0)
                {
                    ...
                    string stringDescription = $"**Country: ** #{user.Country}" + "\n" +
                                               $"**:globe_with_meridians: Global Rank: ** #{user.PPRank} ({user.Country}#{user.PPCountryRank})" + "\n" +
                                               $"**PP: ** {user.PPRaw}" + "\n" +
        ...
        public async Task OsurecentCommand(InteractionContext ctx, [Option("name", "Tên của người bạn cần xem là ai nè")] string name)
        {
            ...
            try
            {
                ...
                if (users != null && users.Length > 0)
                {
                    ...
                    string resultString = $"**🏆 Rank:** {Util.getOsuRank(user.Rank, user.EnabledMods.ToString(), ctx)}\n" +
                                          ...
                                          [Option("mode", "Chọn loại mode mà bạn muốn xem")] BeatmapMode mode)
        {
            ...
            try
            {
                ...
                foreach (var userBestScore in userBestScores)
                {
                    ...
                    string rankEmoji = Util.getOsuRank(userBestScore.Rank.ToString(), mode.ToString(), ctx);
                ...
                embed.WithTitle($"{name} - {user.GetUrl()}");
        ...
        public async Task CheckBeatMapInformationCommand(InteractionContext ctx, [Option("ID","Map bạn cần xem có mã id là gì?")] long id)
        {
            ...
            try
            {
                ...
                {
                    string approvedDate = Util.FormatDate(beatmapInformation.ApprovedDate);
                    string lastUpdate = Util.FormatDate(beatmapInformation.LastUpdate);
                    string submitDate = Util.FormatDate(beatmapInformation.SubmitDate);
